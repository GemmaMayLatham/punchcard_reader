...
byte readADCs(void)
{
  ...
  //temp fix for non analog ports
  if(digitalRead(REG)){adc[8]=1023;}else{adc[8]=0;}
  if(digitalRead(D7)){adc[7]=1023;}else{adc[7]=0;}
  if(digitalRead(D6)){adc[6]=1023;}else{adc[6]=0;}
  if(digitalRead(D5)){adc[5]=1023;}else{adc[5]=0;}
  if(digitalRead(D4)){adc[4]=1023;}else{adc[4]=0;}
  if(digitalRead(D3)){adc[3]=1023;}else{adc[3]=0;}
  if(digitalRead(D2)){adc[2]=1023;}else{adc[2]=0;}
  if(digitalRead(D1)){adc[1]=1023;}else{adc[1]=0;}
  if(digitalRead(D0)){adc[0]=1023;}else{adc[0]=0;}
...
long unsigned int pulseCounter=0;

void writeLEDs(byte reg, byte data)
{
  // Show card data on feedback LEDs - these have been pulsed to increase LED lifetime.
  if(pulseCounter%8==0){
    ...
    digitalWrite(LED_REG, 0);
    digitalWrite(LED_D7, 0);
